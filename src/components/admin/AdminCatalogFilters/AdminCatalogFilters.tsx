...
import styles from "./AdminCatalogFilters.module.css";
...
export const AdminCatalogFilters = () => {
  ...
  useEffect(() => {
    ...
    if (categoryFromUrl) {
      ...
      const event = new CustomEvent("adminCategoryChange", {
  ...
  const toggleSize = (size: string) => {
    ...
    const event = new CustomEvent("adminSizeChange", {
      detail: newSelectedSize,
    });
    window.dispatchEvent(event);
  };
  ...
  const toggleColor = (color: string) => {
    ...
    const event = new CustomEvent("adminColorChange", {
      detail: newSelectedColors,
    });
    window.dispatchEvent(event);
  };
  ...
  const handlePriceChange = (type: "min" | "max", value: string) => {
    ...
    const event = new CustomEvent("adminPriceChange", {
      detail: {
  ...
  useEffect(() => {
    ...
    const pendingEvent = window.dispatchEvent(
      new CustomEvent("checkAdminCategoryChange")
    );
    if (pendingEvent) {
  ...
  const handleCategorySelect = (category: string | null) => {
    ...
    // Emitir evento con la categoría seleccionada
    const event = new CustomEvent("adminCategoryChange", { detail: category });
    window.dispatchEvent(event);
    // Hacer scroll al inicio de la página
    window.scrollTo(0, 0);
  };
